...
wget -q -O /etc/nginx/nginx.conf "${Repo}nginx.conf.txt" 
mkdir -p /home/vps/public_html
wget -q -O /etc/nginx/conf.d/vps.conf "${Repo}/vps.conf.txt"

wget -q -O xraymode.sh ${Repo}Xray%20mode/xraymode.sh && chmod +x xraymode.sh && ./xraymode.sh
sleep 1 
wget -q -O xray.conf ${Repo}xray.conf.txt && chmod +x xray.conf && ./xray.conf
sleep 1 
